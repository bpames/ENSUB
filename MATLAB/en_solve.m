...
% Initial u.
mtilde = ceil(sqrt(2)*m);
rsum = sum(A,2);
u = zeros(M, 1);
[~, rinds] = sort(rsum,"descend");
u(rinds(1:mtilde)) = m/mtilde;

% Initial v.
ntilde = ceil(sqrt(2)*n);
rsum = sum(A,1);
v = zeros(N, 1);
[~, rinds] = sort(rsum,"descend");
v(rinds(1:ntilde)) = n/ntilde;
...
while iter < maxiter
  ...
  xold = x;
  x = soft_thresh((rho*y - lambs - Q*y)/rhohat, gamma*alpha/rho);
...
%   size(xold), size(x), size(yold), size(y)
  ...
  fval = gamma*(alpha*norm(x,1) + (1-alpha)/2*norm(x,2)^2) + 0.5*y'*Q*y;
